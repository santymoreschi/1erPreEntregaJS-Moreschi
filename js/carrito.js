...
if (cartItems.length > 0) {
    ...
    carritoContainer.innerHTML += `
        ...
        <div class="d-flex">
        <button type="button" onclick="restarProducto('${
        i.id
        }')" class="btn btn-secondary rounded-end-0 m-0 border-end-0" data-bs-toggle="button">-</button>
        <button type="button"  class="btn btn-primary active rounded-0" data-bs-toggle="button disabled" aria-pressed="true">${
        i.quantity
        }</button>
        <button type="button" onclick="sumarProducto('${
        i.id
        }')"  class="btn btn-secondary rounded-start-0 m-0 border-start-0" data-bs-toggle="button">+</button>
        </div>
        <button type="button" onclick="eliminarProducto('${
        i.id
        }')" class="btn btn-danger mt-2">Eliminar producto</button>
        </div>
        </div>
    `;
    });
